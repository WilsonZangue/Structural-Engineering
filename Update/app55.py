...
def feature_engineer_data(df_analytics):
    ...
    if 'date_logged' in df_analytics.columns and not pd.api.types.is_datetime64_any_dtype(df_analytics['date_logged']):
        df_analytics['date_logged'] = pd.to_datetime(df_analytics['date_logged'])
    ...
    for start_col, end_col, dur_col in date_candidates:
        if start_col in df_modeling.columns and end_col in df_modeling.columns:
            try:
                df_modeling[start_col] = pd.to_datetime(df_modeling[start_col], errors='coerce')
                df_modeling[end_col] = pd.to_datetime(df_modeling[end_col], errors='coerce')
    ...
    if 'month_started' not in df_modeling.columns or 'quarter' not in df_modeling.columns:
        ...
        if date_source is not None:
            df_modeling[date_source] = pd.to_datetime(df_modeling[date_source], errors='coerce')
...
def train_and_tune_model(X_train, y_train, model_choice, use_quantile):
    """Trains a tuned model using Randomized Search CV, Linear Regression, or Logistic Regression."""
    ...
    else: # Linear Regression, Random Forest, or Standard Gradient Boosting (point mode)
...
def get_shap_data(_models, X_train):
    ...
    # --- CRITICAL FIX: Handle StackingRegressor for SHAP ---
    if isinstance(final_estimator, StackingRegressor):
        # shap.TreeExplainer cannot explain StackingRegressor or Ridge meta-model directly.
        # We extract the first base estimator (Random Forest) to act as the proxy for explanation.
        # This provides a close approximation of feature importance.
        final_estimator = final_estimator.estimators_[0]

    # Create an independent background data for SHAP
    if isinstance(final_estimator, (LinearRegression, Ridge)):
...
def get_shap_interaction_data(_models, X_train):
    ...
    feature_names = preprocessor.transformers_[0][2] + list(
        preprocessor.transformers_[1][1].named_steps['onehot'].get_feature_names_out(
            input_features=['project_type', 'material_type']
        )
    )
...
def run_data_quality_checks(df_raw, df_model):
    """Basic data quality and metadata checks for reporting."""
    required_cols = [
        'project_id', 'project_type', 'material_type', 'surface_area_m2', 'num_levels',
        'scope_category', 'num_units', 'building_height_m', 'floor_area_ratio',
        'planned_start_date', 'planned_end_date', 'expected_duration_days',
        'corrected_start_date', 'corrected_end_date', 'actual_duration_days',
        'num_site_visits', 'project_duration_days', 'month_started', 'quarter',
        'season_flag', 'holiday_period_flag', 'total_project_effort',
        'design_hours_total', 'avg_hours_per_employee', 'num_revisions'
    ]
    ...
    range_checks = {
        ...
        'floor_area_ratio': (0, None),
        'expected_duration_days': (0, None),
        'project_duration_days': (0, None),
        'actual_duration_days': (0, None),
        'num_site_visits': (0, None),
        'design_hours_total': (0, None),
        'avg_hours_per_employee': (0, None),
...
with tabs[0]:
    ...
    missing_df, issues, meta_issues = run_data_quality_checks(df_analytics, df_modeling)
...
with tabs[1]:
    ...
    X = df_modeling[NUM_FEATURES + CAT_FEATURES]
    ...
    with col_m1:
        ...
        model_choice = st.selectbox("Algorithm", ["Linear Regression", "Random Forest", "Gradient Boosting", "Stacking Regressor"], key='model_select')
        use_quantile = st.checkbox(
            "Use Quantile Regression (Risk Intervals)",
            value=True,
            key='use_quantile',
            disabled=model_choice != "Gradient Boosting"
        )
        
    with col_m2:
        st.subheader("Hyperparameters")
        # Disabled for Stacking
        is_random_search_disabled = use_quantile or model_choice in ["Stacking Regressor", "Linear Regression"]
        num_iters = st.number_input("Random Search Iterations (n_iter)", 5, 20, 10, disabled=is_random_search_disabled, key='num_iters')
        
        if model_choice == "Linear Regression":
            st.info("No tuning required (closed-form solution).")
        elif model_choice == "Random Forest":
...
with tabs[2]:
    ...
    if st.session_state['models'] is None:
        ...
        if st.session_state['mode'] == 'quantile':
            model_pipe_for_prep = st.session_state['models']['Q50']
        else:
        ...
        if interaction_values is not None:
            ...
            if rows:
                ...
                pair_labels = [f"{r['Feature A']} × {r['Feature B']}" for _, r in inter_df.iterrows()]
                ...
                if pair_choice != "Auto-Detect (Strongest Interaction)":
                    selected_pair = pair_choice.split(" × ")
        ...
        if view == "SHAP":
            ...
            if dependence_feature in ['project_type', 'material_type'] or dependence_feature.startswith('project_type_') or dependence_feature.startswith('material_type_'):
                ...
                if st.session_state['mode'] == 'quantile':
                    model_pipe_for_ale = st.session_state['models']['Q50']
                else:
        ...
        cap_text = f"This view shows the effect of **{dependence_feature}** on predicted effort. Color indicates **{interaction_feature_name}**."
        st.caption(cap_text)
            ...
            if st.session_state['models'] is None:
                ...
                with st.spinner("Running AI Prediction..."):
                    ...
                    if ss_mode == 'quantile':
                        ...
                        # We use the generic 'models' object which is a single pipeline here
