...
# --- MAIN DASHBOARD ---
st.title("🏗️ Daskan Inc. - Intelligent Effort Estimation System")
st.markdown("This system uses **Machine Learning** to predict project hours based on historical structural data.")

# Create Tabs for the Workflow
tab1, tab2, tab3 = st.tabs(["📊 Data Exploration (EDA)", "⚙️ Model Training", "🔮 Prediction Interface"])
...
with tab3:
    ...
    if 'trained_model' not in st.session_state:
        ...
        if st.button("Estimate Effort"):
            ...
            st.divider()
            st.subheader(f"⏱️ Estimated Effort: {prediction:.1f} Hours")
            
            # Risk Classification (Logic: If > 200 hours, it's 'High Risk')
            if prediction > 200:
                st.error("⚠️ High Risk Project: Requires Senior Supervision")
            else:
