...
with st.sidebar:
    ...
    st.markdown(f"### 🏗️ Daskan Intelligence")
    st.caption("v5.2 - Enterprise AI Estimator (Added Correlation Caption & Data Drift)")
    st.divider()
...
with tabs[0]:
    ...
    st.markdown("### 🕵️ Project Timeline Analysis (S-Curve)")
...
with tabs[1]:
    ...
    if st.session_state['models'] is not None and st.session_state.get('y_preds') is not None:
        ...
        if current_r2 > r2_threshold:
            ...
            button_label = f"⚠️ Review Model Performance (R² < {r2_threshold:.3f})"
            is_safe = False
    ...
    if st.session_state['approved_model_version'] is None and st.session_state['models'] is None:
        ...
        if st.session_state.get('quote_generated', False):
            ...
            if real_duration > 15 or (st.session_state['mode'] == 'quantile' and risk_margin > 50) or is_drifting:
                st.error("⚠️ **High Risk Alert:** Long duration, high uncertainty, or data drift detected. Increase team size or pad the quote.")
            elif i_winter == 1:
                st.info("❄️ **Seasonal Note:** The 'is_winter' factor was included in the estimation.")
            else:
